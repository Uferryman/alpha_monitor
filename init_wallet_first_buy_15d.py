...
# 1. 不保存原始Swap
# 2. 不保存每笔交易
# 3. 只保存 Token + Wallet + first_buy_time
# 4. 普通Token自动合并查询
# 5. 大Token自动拆分
...
# 为什么单独建一张？
#
# wallet_token_first_buy以后可能有几十万行。
#
# 如果每一行都重复存：
#
# symbol
...
cursor.execute(
    """
    CREATE TABLE IF NOT EXISTS wallet_token_first_buy (
...
cursor.execute(
    ...
    idx_wallet_token_first_buy_first_buy

    ON wallet_token_first_buy(
        first_buy_time
    )
    """
)
...
cursor.execute(
    """
    CREATE TABLE IF NOT EXISTS wallet_first_buy_init_checkpoint (
...
cursor.execute(
    """
    CREATE TABLE IF NOT EXISTS wallet_first_buy_init_meta (
...
def get_meta(key):
    ...
    cursor.execute(
        ...
        FROM wallet_first_buy_init_meta
...
def set_meta(
    ...
    cursor.execute(
        """
        INSERT INTO wallet_first_buy_init_meta (
            meta_key,
            meta_value
        )
...
init_status = get_meta(
    "wallet_first_buy_15d_init_status"
)
...
def get_checkpoint(
    ...
    cursor.execute(
        ...
        FROM wallet_first_buy_init_checkpoint
...
def save_token_split_checkpoint(
    ...
    cursor.execute(
        ...
        wallet_first_buy_init_checkpoint (
...
def save_success_segment(
    ...
    try:
        ...
        cursor.executemany(
            """
            INSERT INTO wallet_token_first_buy (
                ...
                first_buy_time =
                    CASE
                        ...
                        WHEN
                            ...
                            <
                            wallet_token_first_buy.first_buy_time
    

                        THEN excluded.first_buy_time

                        ELSE wallet_token_first_buy.first_buy_time
...
# 保存初始化完成到哪个时间点
set_meta(
    "wallet_first_buy_initialized_until",
    init_end,
)
